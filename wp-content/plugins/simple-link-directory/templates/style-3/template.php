<?php wp_enqueue_style('sld-css-style-3' ); ?>
...
if ( $list_query->have_posts() ) 
{
	...
	while ( $list_query->have_posts() ) 
	{
		...
		$customcss = '';
		$customcss .= '#qcopd-list-'.$listId .'-'. get_the_ID().'.style-3 .ca-menu li .ca-main {';
		if($title_font_size!=''){
		...
		$customcss .= '}';
		$customcss .= '#qcopd-list-'. $listId .'-'. get_the_ID().'.style-3 .ca-menu li .ca-sub {';
		...
		$customcss .= '}';
		wp_add_inline_style( 'sld-css-style-3', $customcss );
