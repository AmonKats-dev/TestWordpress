...
add_action( 'admin_menu', 'tecc_add_admin_menu',203 );
...
function tecc_select_field_0_render(  ) { 
    ...
    <select name='tecc_settings[event_id]'>    
	    ...
	    if(is_array($events) && array_filter($events) != null){
		    ...
		    ?>
		    <option value="0"><?php _e('No Future Event found.','tecc1');?></option>
		    <?php 
		} ?> 
	</select>
    <?php
...
function tecc_text_field_1_render(  ) { 
	...
	$options = get_option( 'tecc_settings' );
	?>
	<input type='text' name='tecc_settings[backgroundcolor]' value="<?php echo $options['backgroundcolor']; ?>" class="wp-color-picker-field" data-default-color ="#4395cb">
	<?php
	}
...
function tecc_text_field_2_render(  ) { 
	...
	$options = get_option( 'tecc_settings' );
	?>
	<input type='text' name='tecc_settings[font-color]' value="<?php echo $options['font-color']; ?>" class="wp-color-picker-field" data-default-color ="#ffffff">
	<?php
...
function tecc_select_field_3_render(  ) { 
	...
	<select name='tecc_settings[show-seconds]'>
		<option value="yes" <?php selected( $options['show-seconds'], "yes" ); ?> >Yes</option>
		<option value="no" <?php selected( $options['show-seconds'], "no" ); ?> >No</option>
	</select>
	<?php
		...
		<ul class="main">
			...
			<ul>		
				<?php foreach ( $events as $event ) {
					...
					<li>
						<input class="tecc-checkbox" type='checkbox' name='tecc_settings[future-events-list][]' value="<?php echo $event->ID;?>" <?php echo $checked ?> ><label><?php echo $event->post_title;?></label>
					</li>
				<?php 
				}	?>
	    	</ul>
...
function tecc_settings_section_callback(  ) { 
	echo '<h3>'.__( "Countdown Settings", "tecc1" ).'</h3>';
}
 	...
 	<div class="wrap tecc-from-wrapper">
		...
		if ( isset( $_GET['settings-updated'] ) ) {
            ...
            if(isset($options['event_id']) && !empty($options['event_id']) && $options['event_id']!=0){
				$dynamic_attr='';			
	            $dynamic_attr.="[events-calendar-countdown id=\"{$options['event_id']}\" backgroundcolor=\"{$options['backgroundcolor']}\" font-color=\"{$options['font-color']}\" show-seconds=\"{$options['show-seconds']}\" size=\"{$options['size']}\" event-start=\"{$options['event-start']}\" event-end=\"{$options['event-end']}\" autostart-next-countdown=\"{$options['autostart-next-countdown']}\" autostart-text=\"{$options['autostart-text']}\" autostart-future-countdown=\"{$options['autostart-future-countdown']}\" future-events-list=\"{$k}\" main-title=\"{$options['main-title']}\"";
				$dynamic_attr.=']';

				echo '<h3>'.__("Shortcode Preview","tecc1").'</h3>';
				echo do_shortcode($dynamic_attr);
				$prefix="_tec_";
				echo '<h2>'.__("Countdown for the events calendar Shortcode :","tecc1").'</h2>';		
				...
				echo"</code>";
              	...
              	echo '<h3 style="color:red">'.__("There is no upcoming event. Please add atleast one upcoming event to generate countdown.","tecc1").'</h3>';
            }
   		}  ?>
    	</div>
    </div>
