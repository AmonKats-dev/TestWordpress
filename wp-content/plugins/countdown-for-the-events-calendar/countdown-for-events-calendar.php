...
if (!defined('TECC_VERSION_CURRENT')){
    define('TECC_VERSION_CURRENT', '1.2');
}
    ...
    class EventsCalendarCountdown {
		...
		*/				
		...
		public function Install_TECC_Notice()
		{
			...
			echo '<div class="error CTEC_Msz"><p>' . sprintf( __( 'In order to use our plugin, Please first install the latest version of <a href="%s" class="thickbox" title="%s">%s</a> and add an event.', 'tecc1' ), esc_url( $url ), esc_attr( $title ),esc_attr( $title ) ) . '</p></div>';
			}
		}
		...
		public function tecc_settings_page($links){
			$links[] = '<a style="font-weight:bold" href="'. esc_url( get_admin_url(null, 'edit.php?post_type=tribe_events&page=countdown_for_the_events_calendar') ) .'">'.__("Settings","tecc").'</a>';
			return $links;
		}
			...
			public  function activate() {
				...
				update_option("tecc-installDate",date('Y-m-d h:i:s') );
